<template>
  <div class="container">
    ...
    <div class="prazan">
      <h1 class='naslov'>Zbroj iznosa prema predikciji otkazanih rezervacija</h1>
    </div>
  ...
  <div class="column">
    <div class="vertical">
      <table>
        ...
        <tbody>
            <tr v-for="iznos in iznosi" :key="iznos.id">
              <td hidden="true">{{iznos.id}}</td>
              <td class="siri2"><b>{{iznos.datum}}</b></td>
              <td class="siri">{{(iznos.A21).toFixed(2)}}</td>
              <td class="siri">{{(iznos.A2).toFixed(2)}}</td>
              <td class="siri">{{(iznos.B2).toFixed(2)}}</td>
              <td class="siri">{{(iznos.E1).toFixed(2)}}</td>
              <td class="siri">{{(iznos.D1).toFixed(2)}}</td>
              <td class="siri">{{(iznos.HA).toFixed(2)}}</td>
            </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  methods: {
    getData() {
      axios
        .get('http://127.0.0.1:8080/iznosi')
        .then((response) => {
          console.log(response);
          const detalji = this;
          detalji.iznosi = response.data.data;
        });
    },
    vratiNaHome() {
