<template>
  <div class="container">
    ...
    <div class="prazan">
      <h1 class='naslov'>Predviđeni broj otkazanih jedinica</h1>
    </div>
  ...
  <div class="column">
    <div class="vertical">
      <table>
        ...
        <tbody>
            <tr v-for="jedinica in jedinice" :key="jedinica.id">
              <td hidden="true">{{jedinica.id}}</td>
              <td class="siri2"><b>{{jedinica.datum}}</b></td>
              <td class="siri">{{jedinica.A21}}</td>
              <td class="siri">{{jedinica.A2}}</td>
              <td class="siri">{{jedinica.B2}}</td>
              <td class="siri">{{jedinica.E1}}</td>
              <td class="siri">{{jedinica.D1}}</td>
              <td class="siri">{{jedinica.HA}}</td>
            </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  methods: {
    getData() {
      axios
        .get('http://127.0.0.1:8080/jedinice')
        .then((response) => {
          console.log(response);
          const detalji = this;
          detalji.jedinice = response.data.data;
        });
    },
    vratiNaHome() {
